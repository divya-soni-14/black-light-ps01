...
export default function ImageGrid() {
  ...
  let standardArray = [];
  for (let i = 0; i < 30; i++) standardArray.push(0);
  ...
  async function getRandomImages(count) {
    const response = await axios.get(
      `https://api.unsplash.com/photos/random?client_id=${ACCESS_KEY}&count=${count}&orientation=squarish`
    );
    return response;
  }

  // let images = [
  //   { imgsrc: logo, selected: true, number: "1" },
  //   { imgsrc: logo, selected: true, number: "2" },
  //   { imgsrc: logo, selected: true, number: "3" },
  //   { imgsrc: logo, selected: true, number: "4" },
  //   { imgsrc: logo, selected: true, number: "5" },
  //   { imgsrc: logo, selected: true, number: "6" },
  // ];
  ...
  const clickHandler = (imageNumber) => {
    ...
    if (index == -1) {
      ...
      if (i == 5) console.log("Cannot select more");
      else {
        ...
        {order.map((index) => {
          ...
          if (activeList[index])
            return (
              ...
              >
                <ImageCard
                  imgsrc={imageData[index].urls.small}
